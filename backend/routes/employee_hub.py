...
class ExpenseSheet(BaseModel):
    """Expense sheet submission - employees can create anytime"""
    sheet_date: Optional[str] = None  # Date of the expense sheet
...
async def get_expense_sheets(user_id: Optional[str] = None, month: Optional[int] = None, 
    ...
    cursor = db.expense_sheets.find(query).sort([("created_at", -1)])
    sheets = []
    async for doc in cursor:
