...
const Layout = () => {
  ...
  // ============ DEPARTMENTS HUB ============
  // Projects Department
  // Note: Order Summary, Weekly Billing, and Project Management moved to Business Hub (Mar 17, 2026)
          ...
          <div>
            ...
            {expandedSections.businessHub && (sidebarOpen || isMobile) && (
              <div className="mt-2 space-y-1 ml-2">
                ...
                {/* Vendor Management */}
                {(isSuperAdmin || hasSubModuleAccess('vendor_management_biz')) && (
                <Link
                  to="/business-hub/vendors"
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                    location.pathname === '/business-hub/vendors'
                      ? 'bg-indigo-600/20 text-indigo-400'
                      : 'text-slate-400 hover:bg-slate-800 hover:text-slate-300'
                  }`}
                >
